...
import java.util.*;
import ads.lab2.part3.ListQueue;
...
public class GenealogyTree extends BinaryNode<String> {
	...
	public List<String> ancestors(int level) {
		...
		ancestors(this, 0, level, ancestorsNames);
	...
	public List<String> maleAncestors(int level) {
		List<String> maleAncestorsNames= new ArrayList<>();
		maleAncestors(this, 0, level, maleAncestorsNames);
		return maleAncestorsNames;
	}
	...
	public void displayGenerations() throws EmptyQueueException {
		List<ListQueue<String>> persons = new ArrayList<>();

		generations(this, 0, persons);

		for(ListQueue<String> generation : persons) {
			System.out.println(generation.toString().replaceAll("<-", "").trim());
		}
	}

	private void generations(BinaryNode<String> t, int currentLevel, List<ListQueue<String>> persons) {
		if (t == null) {
			return;
		}

		try {
			persons.get(currentLevel);
		} catch (IndexOutOfBoundsException e) {
			persons.add(new ListQueue<>());
		} finally {
			persons.get(currentLevel).enqueue(t.data());
		}

		generations(t.left(), currentLevel + 1, persons);
		generations(t.right(), currentLevel + 1, persons);
	}
